...
describe('DanbooruService', () => {
  ...
  describe('processRequest', () => {
    ...
    it('should handle processing errors and add to DLQ', async () => {
      ...
      mockDanbooruServiceInstance.publishResponse = jest
        .fn()
        .mockRejectedValue(publishError)

      await expect(
        service.processRequest(jobId, query, clientId),
    ...
    it('should use configurable limit from config', async () => {
      ...
      expect(mockApiService.fetchPosts).toHaveBeenCalledWith(query, 5, true)
    })
  ...
  describe('publishResponse', () => {
    ...
    it('should include timestamp in published response', async () => {
      ...
      const publishedData = JSON.parse(
        (mockRedis.xadd as jest.Mock).mock.calls[0][5] as string,
      )
    ...
    it('should handle processing error and add to DLQ', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          `Error processing job ${jobId}: Processing failed`,
        ),
        jobId,
      )
