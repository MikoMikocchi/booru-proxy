...
export class DanbooruApiService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (retryCount < maxRetries && (error.code === 'ECONNABORTED' || (error.response?.status && error.response.status >= 500))) {
  ...
  async fetchPosts(query: string, limit: number = 1, random: boolean = true): Promise<DanbooruPost | null> {
    ...
    try {
      ...
      const errors: ValidationError[] = await validate(post, { forbidNonWhitelisted: true })
      if (errors.length > 0) {
        this.logger.warn(`Validation errors in Danbooru response: ${JSON.stringify(errors)}`)
        return null
      }
      ...
      this.logger.error(`API error for query ${query}: ${(error as Error).message}`)
      return null
    }
  }
