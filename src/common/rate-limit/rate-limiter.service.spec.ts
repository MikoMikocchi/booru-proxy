...
describe('RateLimiterService', () => {
  ...
  beforeEach(async () => {
    const mockPipeline: MockPipeline = {
      _commands: [] as any[],
      eval: jest.fn((script: string, keys: number, ...args: any[]): MockPipeline => {
  ...
  describe('checkRateLimit', () => {
    ...
    it('should block request when over limit', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Rate limit exceeded for danbooru key rate:danbooru:user123'),
      );
    });
  ...
  describe('checkCompositeRateLimit', () => {
    ...
    it('should allow when all individual limits are under threshold', async () => {
      ...
      const result = await service.checkCompositeRateLimit('danbooru', ['ip:192.168.1.1', 'user:user123'], 5, 60);
    ...
    it('should call eval for each identifier in pipeline', async () => {
      ...
      await service.checkCompositeRateLimit('danbooru', ['ip1', 'ip2', 'ip3'], 10, 60);
  ...
  describe('resetRateLimit', () => {
    it('should reset specific clientId', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith('Reset rate limit for danbooru user123');
    });
    ...
    it('should reset global counter', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith('Reset rate limit for danbooru global');
    });

    it('should handle non-existent key', async () => {
      mockRedis.del.mockResolvedValueOnce(0);

      await expect(service.resetRateLimit('danbooru', 'nonexistent')).resolves.toBeUndefined();
      expect(mockLogger.log).toHaveBeenCalledWith('Reset rate limit for danbooru nonexistent');
    });
  });
