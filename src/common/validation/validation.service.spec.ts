...
import { ValidationService, ValidationResult, ValidationError } from './validation.service';
...
const mockPlainToClass = classTransformer.plainToClass as jest.MockedFunction<typeof classTransformer.plainToClass>;
const mockValidate = classValidator.validate as jest.MockedFunction<typeof classValidator.validate>;
...
describe('ValidationService', () => {
  ...
  describe('validateRequest', () => {
    ...
    it('should validate successfully when all checks pass', async () => {
      ...
      // Mock authentication
      jest.spyOn(service as any, 'validateAuthentication').mockResolvedValue(null);
      ...
      const result = await service.validateRequest(validJobData, TestDto, config) as ValidationResult<TestDto>;
      ...
      if (result.valid) {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith('Validation successful for danbooru job test-job-123');
        expect(mockRateLimitManager.checkRateLimit).toHaveBeenCalledWith('danbooru', 'test-job-123', 'user123');
      } else {
    ...
    it('should fail on DTO validation errors', async () => {
      ...
      if (!result.valid) {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringContaining('DTO validation failed for danbooru job test-job-123'),
        );
    ...
    it('should fail on authentication validation', async () => {
      ...
      jest.spyOn(service as any, 'validateAuthentication').mockResolvedValue(authError);
    ...
    it('should fail on custom validator error', async () => {
      ...
      if (!result.valid) {
        ...
        expect(config.customValidator).toHaveBeenCalledWith(validJobData, config);
        expect(mockRateLimitManager.checkRateLimit).not.toHaveBeenCalled();
      } else {
        fail('Validation should have failed');
      }
    ...
    it('should fail on rate limit validation', async () => {
      ...
      if (!result.valid) {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringContaining('Rate limit validation failed for danbooru job test-job-123'),
        );
      } else {
        fail('Validation should have failed');
      }
    ...
    it('should handle missing jobId gracefully', async () => {
      ...
      const result = await service.validateRequest(noJobIdData, TestDto, config) as ValidationResult<TestDto>;

      if (result.valid) {
        expect(result.valid).toBe(true);
        expect(mockRateLimitManager.checkRateLimit).toHaveBeenCalledWith('danbooru', 'unknown', 'user123');
      } else {
        fail('Validation should have passed');
      }
    });
    ...
    it('should skip rate limit if no clientId provided', async () => {
      ...
      const result = await service.validateRequest(noClientIdData, TestDto, config) as ValidationResult<TestDto>;

      if (result.valid) {
        expect(result.valid).toBe(true);
        expect(mockRateLimitManager.checkRateLimit).toHaveBeenCalledWith('danbooru', 'test-job-123', 'no-client');
      } else {
        fail('Validation should have passed');
      }
    });
  ...
  describe('validateAuthentication', () => {
    ...
    it('should pass validation with correct HMAC', async () => {
      ...
      const result = await (service as any).validateAuthentication(jobData, config);
    ...
    it('should fail validation with missing API key', async () => {
      ...
      const result = await (service as any).validateAuthentication(noApiKeyData, config);
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('Missing API key for danbooru job test-job-123');
    ...
    it('should fail validation with invalid HMAC', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('Invalid HMAC for danbooru job test-job-123');
    });
  ...
  describe('validateRateLimit', () => {
    it('should pass when rate limit allows', async () => {
      ...
      const result = await (service as any).validateRateLimit('danbooru', 'test-job-123', 'user123');
    ...
    it('should handle rate limit without clientId', async () => {
      ...
      const result = await (service as any).validateRateLimit('danbooru', 'test-job-123');

      expect(result).toBeNull();
      expect(mockRateLimitManager.checkRateLimit).toHaveBeenCalledWith('danbooru', 'test-job-123', undefined);
    });
  });
  ...
  describe('integration scenarios', () => {
    it('should handle complete validation flow with all components', async () => {
      ...
      mockRateLimitManager.checkRateLimit.mockResolvedValue({ allowed: true } as any);

      const result = await service.validateRequest(fullJobData, TestDto, fullConfig) as ValidationResult<TestDto>;
      ...
      if (result.valid) {
        ...
        expect(fullConfig.customValidator).toHaveBeenCalledWith(fullJobData, fullConfig);
        expect(mockRateLimitManager.checkRateLimit).toHaveBeenCalledWith('danbooru', 'integration-test-1', 'integration-user');
    ...
    it('should short-circuit on first validation failure', async () => {
      const failingJobData: { [key: string]: string } = { jobId: 'short-circuit-test' };
  ...
  describe('error formatting', () => {
    it('should create proper ValidationError objects', async () => {
      ...
      const dtoResult = await service.validateRequest(jobData, TestDto, config) as ValidationResult<TestDto>;
      if (!dtoResult.valid) {
      ...
      const rateResult = await service.validateRequest(jobData, TestDto, config) as ValidationResult<TestDto>;
      if (!rateResult.valid) {
