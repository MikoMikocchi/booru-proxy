...
export class CacheManagerService {
  ...
  async getOrFetch<T extends CacheableResponse>(
    ...
    if (!random) {
      cached = await this.cacheService.getCachedResponse<T>(apiPrefix, query, random);
      if (cached) {
        this.logger.log(`Cache hit for ${apiPrefix} job ${jobId}: query length ${query.length}`);
        return cached;
      }
    }

    // Cache miss or random query - fetch from source
    this.logger.debug(`${apiPrefix} cache miss for job ${jobId}, fetching fresh data`);
    const freshData = await fetchFn();

    if (freshData && !random) {
      // Cache successful non-random responses
      await this.cacheService.setCache(apiPrefix, query, freshData, random, customTtl);
  ...
  async cacheResponseIfNeeded<T extends CacheableResponse>(
    ...
    if (!random && response) {
      await this.cacheService.setCache(apiPrefix, query, response, random, customTtl);
    }
  }

  async invalidateCache(apiPrefix: string, query?: string, random?: boolean): Promise<void> {
    if (query && random !== undefined) {
      await this.cacheService.deleteCache(apiPrefix, query, random);
      this.logger.log(`Invalidated specific cache for ${apiPrefix}: ${query} (random: ${random})`);
    } else {
      const deletedCount = await this.cacheService.invalidateByPrefix(apiPrefix);
      this.logger.log(`Bulk invalidated ${deletedCount} cache entries for ${apiPrefix}`);
    }
  }

  // Multi-API support: get cache stats (for monitoring)
  async getCacheStats(apiPrefix: string): Promise<{ hits: number; misses: number; size: number }> {
