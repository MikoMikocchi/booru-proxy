...
export class CacheService {
  ...
  async setCache<T extends CacheableResponse>(
    ...
    this.logger.debug(`Cached response for ${apiPrefix} query: ${query} (random: ${random}, ttl: ${expiresIn}s)`);
  }

  async deleteCache(apiPrefix: string, query: string, random: boolean): Promise<void> {
    const key = this.getCacheKey(apiPrefix, query, random);
    const deleted = await this.redis.del(key);
    if (deleted > 0) {
      this.logger.debug(`Deleted cache for ${apiPrefix} query: ${query} (random: ${random})`);
    }
  }
  ...
  private getCacheKey(apiPrefix: string, query: string, random: boolean): string {
